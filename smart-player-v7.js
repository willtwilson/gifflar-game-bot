/**
 * Gifflar Winter Vibe - Smart Game Player v7
 *
 ...
 *  - Minimum round duration (3s) before checking roundOver. Fixes ~40% of
 *    rounds dying in 1s due to leaderboard animation transient.
 *  - Active QUIT mode: steer toward platform gaps to end stuck rounds quickly,
 *    instead of passively waiting for ball to fall off.
 *  - Vertical oscillation detection: force escape mode if ball bounces on
 *    same 2 X positions 4+ times.
 *  - Emergency preservation: when falling with no candidates, aim for any
 *    platform within 600 units.
 *  - Trampoline priority boost: 700→1200 to always prefer trampolines.
 *  - Escape mode looks up to 2x bounceH above apex (was 1.5x).
 */
...
async function runGame() {
        ...
        window.Phaser.Game = function(...args) {
          ...
          game.events.on('step', () => {
            ...
            if (goingUp && d.wasGoingDown) {
              ...
              // If last 4 bounces are on only 1-2 distinct X buckets → force wander immediately
              if (d.platXHistory.length >= 4) {
                const distinctX = new Set(d.platXHistory.slice(-4)).size;
                if (distinctX <= 2 && d.stagnantBounces < WANDER_AFTER) {
                  d.stagnantBounces = WANDER_AFTER; // trigger escape mode now
                }
              }
            }
            ...
            if (d.stagnantBounces >= WANDER_AFTER) {
              // v7 Fix 3: Active QUIT mode — steer toward platform gaps to end round quickly
              if (d.stagnantScoreBounces >= 10 && d.stagnantBounces >= 8) {
                ai.direction = 'QUIT';
                ai.desiredX = 187;
                ai.platformsAbove = 0;

                // Find the X position below with the largest gap between platforms
                const platsBelow = allPlats.filter(p => p.y > py && p.y < py + 600);
                let gapX = null;
                if (platsBelow.length > 0) {
                  let bestGapX = px, minNearby = Infinity;
                  for (let testX = 50; testX < WORLD_WIDTH; testX += 100) {
                    const nearby = platsBelow.filter(p => {
                      const raw = Math.abs(p.x - testX);
                      return Math.min(raw, WORLD_WIDTH - raw) < 80;
                    }).length;
                    if (nearby < minNearby) { minNearby = nearby; bestGapX = testX; }
                  }
                  gapX = bestGapX;
                }
                ...
                if (gapX !== null) {
                  ...
                  let diff = gapX - px;
                  if (Math.abs(diff) > WORLD_WIDTH / 2) diff = diff > 0 ? diff - WORLD_WIDTH : diff + WORLD_WIDTH;
                  if (Math.abs(diff) > 20) {
                    ...
                    ai.taps++;
                    ai.direction = goRight ? 'QUIT→R' : 'QUIT→L';
                  }
                } else {
                  // No platforms below — just stop steering, ball will fall naturally
                  scene.isTouching = false;
                }

                // Dampen upward velocity to break bounce cycle
                try {
                  const vy = scene.player.getVelocityY();
                  if (vy < 0) { // ball is going up, push it down harder
                    scene.player.setVelocityY(Math.abs(vy) * 0.3);
                  }
                } catch (_) {}
              ...
              // v7 Improvement D: look up to 2.0x bounceH above apex (was 1.5x)
              ...
              const expandedCands = allPlats.filter(p =>
                ...
                p.y > (d.apexY ?? py) - d.bounceH * Math.min(2.0 + expandFactor, 5.0) && // v7: expanded Y reach
            ...
            // v7 Fix 5: Emergency preservation — ball is falling with nothing reachable
            if (candidates.length === 0 && goingDown) {
              const emergency = allPlats.filter(p =>
                Math.abs(p.y - py) < 600 &&
                effectiveXDist(p.x) <= maxReach * 2.5
              ).sort((a, b) => {
                // Prefer: platforms below the ball (to land on), then closest
                const aBelow = a.y > py ? 0 : 1000;
                const bBelow = b.y > py ? 0 : 1000;
                return (aBelow + Math.abs(a.y - py)) - (bBelow + Math.abs(b.y - py));
              });
              if (emergency.length > 0) candidates = [emergency[0]];
            }
            ...
            for (const p of candidates) {
              ...
              const pScore =
                -xEff * 0.8
                + (-p.y) * 0.05
                + (isTrampoline ? 1200 : 0)  // v7: boosted from 700 → 1200
                - (isMoving     ? 30  : 0);
  ...
  // v7 Fix 2: Minimum round duration before trusting roundOver
  ...
  async function playRound(roundNum) {
    ...
    while (Date.now() - roundStart < maxRoundMs) {
      ...
      // v7 Fix 2: Only trust roundOver after MIN_ROUND_MS grace period
      if (snap.roundOver && !screenshotted && (Date.now() - roundStart) > MIN_ROUND_MS) {
        screenshotted = true;
      ...
      if (snap.roundOver && (Date.now() - roundStart) > MIN_ROUND_MS) break;
