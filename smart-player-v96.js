/**
 * Gifflar Winter Vibe - Smart Game Player v9.6
 *
 * Key regression fix: restore v4's x-distance-heavy scoring that achieved 220.1
 * - Scoring: -xEff*0.6 + (-p.y)*0.08 (was 0.3/0.3 — equal weights caused chasing faraway platforms)
 * - Trampoline bonus: 1000 (was 2000 — reduced to stop chasing out-of-reach trampolines)
 * - WANDER_AFTER: 5 (was 3 — less premature escape mode triggering)
 * - Wander mode: pure directional L/R (was complex candidate search — simpler = more reliable)
 * - FALLING fallback: tight range py ± BH (was 2.5×BH below — ball was overshooting)
 * - Moving platform penalty: 50 (was 80)
 */
...
async function runGame() {
  ...
  await page.addInitScript(() => {
    ...
    const WANDER_AFTER  = 5;      // v9.6: less premature escape triggering (was 3)
        ...
        window.Phaser.Game = function(...args) {
          ...
          game.events.on('step', () => {
            ...
            if (d.stagnantBounces >= WANDER_AFTER) {
              ...
              // Pure directional wander — just push L or R to find new platforms.
              // Complex candidate search in wander mode caused erratic over-steering; 
              // simple directional thrust is more reliable.
  ...
  console.log('=== Loading game (v9.6 — v4-scoring-recovery + pure-wander + tight-fallback) ===');
  await page.goto(
