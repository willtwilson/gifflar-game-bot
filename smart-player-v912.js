...
async function runGame() {
        ...
        window.Phaser.Game = function(...args) {
          ...
          game.events.on('step', () => {
            ...
            if (goingUp && d.wasGoingDown) {
              ...
              // Pre-compute effectiveWanderAfter here so oscillation escape can use it
              const oscWanderAfter = (ai.highScore > 80) ? 8 : WANDER_AFTER;
              ...
              if (d.platXHistory.length >= 4 && d.bounceCount >= 8) {
                ...
                if (distinctX <= 2 && d.stagnantBounces < oscWanderAfter) {
                  d.stagnantBounces = oscWanderAfter;  // use dynamic threshold so wander ACTUALLY triggers
                }
              }
            }
            ...
            if (d.stagnantBounces >= effectiveWanderAfter) {
              ...
              const windowEmg = allPlats.filter(p =>
                ...
                effectiveXDist(p.x) < searchRange * 0.8
              ).sort((a, b) => a.y - b.y);     // most negative Y first = highest reachable = most progress

              // Fallback: any platform in 2×BH proximity radius — WITH lateral reach filter.
              // Without the reach filter, nearbyEmg picks platforms 200+ px sideways the ball can never reach.
              const nearbyEmg = allPlats.filter(p =>
                Math.abs(p.y - py) < d.bounceH * 2.0 &&
                effectiveXDist(p.x) < searchRange * 0.8   // must be laterally reachable
              ...
              if (emergencyCands.length > 0) {
                ...
                if (Math.abs(diff) >= STEER_THRESH) {
                  // Target is meaningfully sideways — steer toward it (genuine emergency target).
                ...
                // diff < STEER_THRESH: target is directly above/below — not worth steering to.
                // Fall through to pure directional wander below (same as v9.6, proven effective).
              }
            ...
            if (goingDown || ai.phase === 'apex') {
              ...
              // v9.11 LIFE PRESERVATION: When falling, ONLY look for platforms below the ball.
              // Previously: p.y > py - BH * 0.5 included platforms up to 0.5 BH ABOVE the ball.
              // Ball is falling — it CANNOT land on anything above it. Only below matters.
              if (candidates.length === 0) {
                const emergency = allPlats.filter(p =>
                  p.y > py   // strictly BELOW ball (less negative Y = lower altitude = reachable by falling)
                ).sort((a, b) => a.y - b.y);  // closest below first (smallest Y > py first)
                if (emergency.length > 0) { candidates = [emergency[0]]; ai.direction = 'EMG'; }
              }
              // Last resort: anything reachable if no platform was found below at all
              if (candidates.length === 0) {
                const lastResort = allPlats
                  .sort((a, b) => {
                    const aScore = (a.y >= py ? 0 : 10000) + Math.abs(a.y - py);
                    const bScore = (b.y >= py ? 0 : 10000) + Math.abs(b.y - py);
                    return aScore - bScore;
                  });
                if (lastResort.length > 0) { candidates = [lastResort[0]]; ai.direction = 'EMG-LR'; }
              }
            } else {
            ...
            // ──────────────── v9.11 Life preservation override ────────────────
            // If FALLING and the best-scored target is now ABOVE the ball (ball already flew past it
            // on the way up, or it was selected while rising), immediately switch to the closest
            // platform BELOW. Aiming upward while falling = guaranteed death.
            if (goingDown && bestTarget.y < py) {
              const belowPlats = allPlats
                .filter(p => p.y > py && effectiveXDist(p.x) <= maxReach * 2.0)
                .sort((a, b) => a.y - b.y);  // nearest below first
              if (belowPlats.length > 0) {
                bestTarget = belowPlats[0];
                ai.direction = 'LIFE';  // visible in snapshot for debugging
              } else {
                // Nothing reachable below — steer center and hope for the best
                ai.direction     = 'none(life)';
                ai.desiredX      = 187;
                scene.isTouching = false;
                return;
              }
  ...
  console.log('=== Loading game (v9.12 — QA fixes: world-wrap emergency + oscillation threshold + isCheater logging + recency penalty) ===');
  await page.goto(
