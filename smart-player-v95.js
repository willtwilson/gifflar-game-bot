...
async function runGame() {
          ...
          game.events.on('step', () => {
              ...
              if (expandedCands.length > 0) {
                ...
                for (const p of expandedCands) {
                  ...
                  const tramBonus = isTrampoline
                    ...
                    : 0;
                  // v9.5: upward bias — prefer platforms above current position
                  const upwardBonus = (p.y < py) ? 150 : 0;
                  const pScore =
                    -xEff * 0.3
                    + (-p.y) * 0.3
                    + tramBonus
            ...
            for (const p of candidates) {
              ...
              const pScore =
                ...
                + tramBonus
                - (isMoving ? 80 : 0)  // moving platforms drift — harder to land on
                + reachComfort;
              if (pScore > bestPScore) { bestPScore = pScore; bestTarget = p; }
            }
  ...
  console.log('=== Loading game (v9.5 — time-stagnant-exit + CTB-survival + wander-upward) ===');
  await page.goto(
