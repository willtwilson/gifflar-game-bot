/**
 * Gifflar Winter Vibe - Smart Game Player v9.7
 *
 * Key fix over v9.6: high-altitude survival — ball was dying at score 169 because
 * it entered wander mode at extreme altitude with Cands:0 and no emergency search.
 *  - Altitude-aware WANDER_AFTER: 5 normally, 8 when score > 80 (sparse platform zones)
 *  - Wander emergency search: when Cands:0 in wander mode, try 5×BH radius any platform
 *  - Post-trampoline survival: BH > 500 also triggers wide emergency search
 */
...
async function runGame() {
        ...
        window.Phaser.Game = function(...args) {
          ...
          game.events.on('step', () => {
            ...
            if (d.stagnantBounces >= effectiveWanderAfter) {
              ...
              const isBigBounce = d.bounceH > 450;
              const searchRange = (isHighAlt || isBigBounce) ? d.bounceH * 5.0 : d.bounceH * 2.5;

              const emergencyCands = allPlats.filter(p =>
  ...
  console.log('=== Loading game (v9.7 — high-altitude-survival + wander-emergency-search) ===');
  await page.goto(
