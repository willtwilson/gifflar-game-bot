/**
 * Gifflar Winter Vibe - Smart Game Player v9.8
 *
 * Key fixes over v9.7: above-apex emergency search — ball was targeting below-apex
 * platforms during wander mode near stagnant altitude, overshooting and dying.
 *  - Emergency search now prefers platforms ABOVE apex (can actually land on them)
 *  - WANDER_AFTER raised from 5 → 6 (one more bounce of normal candidate search)
 *  - isBigBounce threshold lowered from 450 → 400 (catches more trampoline bounces)
 */
...
async function runGame() {
        ...
        window.Phaser.Game = function(...args) {
          ...
          game.events.on('step', () => {
            ...
            if (d.stagnantBounces >= effectiveWanderAfter) {
              ...
              // Prefer platforms ABOVE current apex — these are the ones we can actually land on
              const apexRef = d.apexY || py;
              const aboveApexEmg = allPlats.filter(p =>
                p.y < apexRef + 80 &&              // at or above apex
                Math.abs(p.x - px) < searchRange * 0.8  // reachable sideways
              ).sort((a, b) => a.y - b.y);          // highest first (most negative Y)

              // Fallback: any platform in 5×BH radius sorted by proximity
              const nearbyEmg = allPlats.filter(p =>
                Math.abs(p.y - py) < searchRange
              ).sort((a, b) => Math.abs(a.y - py) - Math.abs(b.y - py));

              const emergencyCands = aboveApexEmg.length > 0 ? aboveApexEmg : nearbyEmg;
  ...
  console.log('=== Loading game (v9.8 — above-apex-emergency + wander-after-6 + bigbounce-400) ===');
  await page.goto(
